...
the model to map the design space and select the parts of ROS~2 monitoring that
could reasonably fit into one thesis prototype.
...
configured through YAML and plugins around a stable monitoring core.

\section{Selected Feature Coverage}

Table~\ref{tab:selected-feature-coverage} summarizes how the implemented
prototype relates to the feature model. The table distinguishes between the
feature coverage that is implemented and exercised by the five evaluation
experiments (Chapter~\ref{chapter:evaluation}) and unselected feature
variants.
...
The infrastructure supports an integrated deployment and a split
robot/verifier deployment. The evaluation exercises both forms with one and
two robots on a single machine and across three physical machines. &
...
Single-robot and multi-robot configurations are expressed with the same YAML
configuration concepts and record format; the evaluation generates them from
one deployment description per experiment. &
...
This coverage selects a focused subset of the full feature model. The goal is
to build and validate a reusable path from ROS~2 application observations to
checking components, while keeping the parts that usually change between
...
events, and action feedback/status need a single data shape for later
components. The record therefore carries the payload together with the source,
timestamp, session, and identity information needed to connect observations
with later verdicts.
...
handles ROS~2 types and communication, while checker-specific knowledge stays
in converters and verdict services. A converter may adapt records for a
checker, while a verdict service contains the rule-specific logic. YAML
configuration covers common choices,
...
record for one rule-specific checker. A verdict service evaluates the
...
A converter and a verdict service can also form an adapter for a user-defined
DSL. The converter translates common data records into the input format of the
DSL, and the verdict service holds or calls its checking engine. Both components
can be supplied as plugins. This keeps the DSL and its checking logic outside
the monitoring core while giving every checker the same record, deployment,
and verdict interfaces.
...
records, so the same checking architecture accommodates each source type.
...
\begin{description}
    ...
    \item[Converter] A rule-specific adapter that projects a data record
    into the input shape expected by a checker.

    \item[Verdict service] A checker that evaluates converted data and returns
    a structured verdict when a rule state is observed.
...
adapt records for rule logic, and verdict services perform the actual
checking.

\section{Common Data Record}

The common data record is the central abstraction of the infrastructure. Every
captured topic message, service event, and action feedback or status observation
is converted into this format before it reaches processing, transport, or
checking. Records are serialized as JSONL, with one JSON object per line. Empty
optional fields are omitted from the serialized output to keep files and MQTT
payloads compact.
...
Each topic message is a complete unit and uses a single record form. Service
events
...
The same pipeline can run in two main deployment forms. They are the concrete
instances, in this infrastructure, of the general monitoring organisations
introduced in Section~\ref{sec:monitoring-organisations}.

In the integrated deployment, collection, processing, checking, and output run
in one ROS~2-facing process. This form is useful for local development and for
rules that can be checked close to the observed application. It
corresponds to the traditional organisation in
Figure~\ref{fig:org-traditional}: the application and the complete monitoring
pipeline share one location and one clock. The monitor processes the received
records in a local order and one process analyses this observed trace. DDS
delivery can still differ from publication order when records come from
several publishers.
...
flows. Transport is an exporter/source choice, separate from the checking
logic.

The split deployment is closest to the orchestrated organisation in
Figure~\ref{fig:org-orchestrated}: the trace is produced at the observed
location, while the analysis runs on a remote verifier, so the orchestration
trade-offs from Section~\ref{sec:monitoring-organisations}, communication
overhead and data exposure on the network, apply to it. Because the robot-side
transformer pipeline can already filter and reduce the records before they are
transmitted, the split deployment also borrows the main advantage of the
choreographed organisation: part of the monitoring work stays at the location
where the trace is produced, which lowers the amount of data that crosses the
network.
