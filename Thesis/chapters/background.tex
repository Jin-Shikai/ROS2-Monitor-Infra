...
report whether the observed behaviour satisfies or violates a rule, but it
...
processing runtime data, while runtime verification denotes the rule-checking
...
\begin{description}
    ...
    \item[Rule] An expected condition that the observed behaviour is checked
    against. The literature also calls this a
    \emph{property}~\cite{leucker2009brief}.

    \item[Monitor] A component that observes the system or checks rules.

    \item[Verdict] The result of a rule check.
\end{description}

Figure~\ref{fig:monitoring-terms} shows how these terms relate. The monitor
captures observations from the system under monitoring. Based on a rule,
...
\begin{tikzpicture}[
  ...
  \node[box, dashed, above=1.0cm of monitor] (property) {Rule};
...
from the system under monitoring, checks them against a rule, and reports
a verdict.}
...
After processing, the collected data needs to be sent to the correct
destination. For example, the same observation stream may be written to a
file, sent over a network, and passed to a checking component. Adding or changing such destinations
...
Checking decides whether observed data follows an expected rule. Different
...
that a global rule can often be split into smaller rules that
...
traces and, when they contribute to the same global rule, coordinate with
each other. This reduces data exposure and communication overhead, but it is
more complex to set up: the rules have to be decomposed into coordinated
