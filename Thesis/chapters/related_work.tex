...
The reviewed approaches pursue different goals with overlapping terminology.
To keep them comparable, each approach is described along the same six
questions:
...
events. In online mode it sends events over WebSocket to an external checker
(called an \emph{oracle} in ROSMonitoring); in offline mode it records a
trace. An online filtering mode can block or republish messages according to
the checker result. The property formalism is therefore supplied by the
external checker rather than fixed by the ROS-facing monitor.
...
monitoring-configuration phase from data collection. Reusable
system-under-monitoring types and monitor configurations describe topic
fields, sampling,
...
artifacts. Copilot here denotes a stream-based runtime-verification language
and framework, not the identically named AI programming assistant. Ogma
generates Copilot code, which is compiled to C99 and packaged as a ROS~2 node.
...
form, but it is not equivalent to decentralizing the property itself.
...
\section{Comparison of the Reviewed Approaches}

Table~\ref{tab:related-work-focused} summarizes the reviewed approaches along
the review criteria. A dash means that the publication does not make that
...
\begin{longtable}{
  ...
  >{\raggedright\arraybackslash}p{.18\textwidth}
  >{\raggedright\arraybackslash}p{.18\textwidth}}
\caption{Comparison of the reviewed monitoring approaches.}
...
External checker; 2.0 also provides RML/past-time MTL &
...
ROS~2 observations and property-specific checkers; the next chapter presents
this design.
