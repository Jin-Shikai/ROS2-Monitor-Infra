...
collected without generating source-specific monitoring code. Python also keeps
...
output file can be interpreted later without the original YAML file. On
...
exchanges use services inside ROS~2 actions. The prototype leaves those phases
to service introspection and does not collect them in the action collector.
...
case, ROS~2 can report a placeholder publisher name. The collectors do not
cache such placeholders. They retry the lookup on later callbacks and store the
...
fields, so records can pass through files or network transports without losing
their monitoring context. A data record identifier combines the session
...
no record. Returning no record drops the observation and stops the remaining
...
lines appended to the file. Without \texttt{follow}, it replays the file once,
...
converter, or no output. A converter may also run its own timer, which is
needed for deadline properties. A verdict service evaluates checker-ready data
and may return a verdict. A verdict contains a timestamp, a property
identifier, a boolean result, and details.
...
behind it. Converters and verdict services do not import ROS~2 packages and do
not assume a particular property language.
...
\begin{itemize}
    ...
    \item \textbf{ResetPoseEffectConverter} checks a property over two
    ...
    a violation when the deadline expires.

    \item \textbf{RelativeSpeedConverter} joins odometry streams from two
    robots and emits their relative speed. This demonstrates a property that
...
only converters or verdict services receives a runner configuration. A host
that only runs the observed ROS~2 application does not need a generated file.
...
placements are rejected during generation. For example, a converter on a
monitor host cannot consume a cross-host record stream, because cross-host
record inputs are handled by the runner runtime.
